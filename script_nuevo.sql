...
CREATE INDEX ind_inquilino ON GESTIONATE.inquilino (numero_doc);
CREATE INDEX ind_agente ON GESTIONATE.agente (numero_doc);
CREATE INDEX ind_propietario ON GESTIONATE.propietario (numero_doc);
GO
...
BEGIN
	INSERT INTO GESTIONATE.barrio (nombre, codigo_localidad)
		...
		(SELECT TOP 1 id_localidad FROM GESTIONATE.localidad WHERE nombre=INMUEBLE_LOCALIDAD)
...
BEGIN
	INSERT INTO GESTIONATE.direccion (detalle, codigo_localidad, codigo_provincia, codigo_barrio)
		SELECT u.d,
		(SELECT TOP 1 id_localidad FROM GESTIONATE.localidad WHERE nombre=u.l),
		(SELECT TOP 1 id_provincia FROM GESTIONATE.provincia WHERE nombre=u.p),
		(SELECT TOP 1 id_barrio FROM GESTIONATE.barrio WHERE nombre=u.b)
		FROM
...
BEGIN
	SET IDENTITY_INSERT GESTIONATE.sucursal ON
		...
		SELECT DISTINCT SUCURSAL_CODIGO, SUCURSAL_NOMBRE,
		(SELECT id_direccion FROM GESTIONATE.direccion WHERE detalle=SUCURSAL_DIRECCION),
		SUCURSAL_TELEFONO FROM gd_esquema.Maestra WHERE SUCURSAL_CODIGO IS NOT NULL
...
BEGIN
	SET IDENTITY_INSERT GESTIONATE.inmueble ON
		...
		(DATEADD(year, -INMUEBLE_ANTIGUEDAD, GETDATE())),
		INMUEBLE_EXPESAS FROM gd_esquema.Maestra WHERE INMUEBLE_CODIGO IS NOT NULL
...
BEGIN
	...
	SELECT @id_cable = id_caracteristica FROM GESTIONATE.caracteristica WHERE detalle='CABLE';
	SELECT @id_wifi = id_caracteristica FROM GESTIONATE.caracteristica WHERE detalle='WIFI';
	SELECT @id_calefaccion = id_caracteristica FROM GESTIONATE.caracteristica WHERE detalle='CALEFACCION';
	SELECT @id_gas = id_caracteristica FROM GESTIONATE.caracteristica WHERE detalle='GAS';
...
BEGIN
	SET IDENTITY_INSERT GESTIONATE.anuncio ON
		...
		INMUEBLE_CODIGO, ANUNCIO_PRECIO_PUBLICADO,
		(SELECT id_moneda FROM GESTIONATE.moneda WHERE detalle=ANUNCIO_MONEDA),
...
BEGIN
    INSERT INTO [GESTIONATE].pago_venta
               ...
               SELECT VENTA_CODIGO, PAGO_VENTA_IMPORTE, PAGO_VENTA_COTIZACION,
			   (SELECT id_moneda FROM GESTIONATE.moneda WHERE detalle=PAGO_VENTA_MONEDA),
...
BEGIN
	SET IDENTITY_INSERT GESTIONATE.venta ON
		INSERT INTO [GESTIONATE].[venta]
				   ...
				   (SELECT id_comprador FROM GESTIONATE.comprador WHERE numero_doc=COMPRADOR_DNI), VENTA_FECHA, VENTA_PRECIO_VENTA,
				   (SELECT id_moneda FROM GESTIONATE.moneda WHERE detalle=VENTA_MONEDA),
				   VENTA_COMISION FROM gd_esquema.Maestra m
