...
BEGIN
	INSERT INTO GESTIONATE.direccion (detalle, codigo_localidad, codigo_provincia)
		SELECT DISTINCT SUCURSAL_DIRECCION, GESTIONATE.OBTENER_LOCALIDAD(SUCURSAL_LOCALIDAD), GESTIONATE.OBTENER_PROVINCIA(SUCURSAL_PROVINCIA) FROM gd_esquema.Maestra WHERE SUCURSAL_DIRECCION IS NOT NULL
		UNION
...
BEGIN
    ...
    SELECT GESTIONATE.OBTENER_ALQUILER(ALQUILER_CODIGO), DETALLE_ALQ_NRO_PERIODO_INI, DETALLE_ALQ_NRO_PERIODO_FIN, DETALLE_ALQ_PRECIO
  FROM [GD2C2023].[gd_esquema].[Maestra] WHERE ALQUILER_CODIGO IS NOT NULL
END
GO